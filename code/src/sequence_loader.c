...
static TaskHandle task_handle_trig = 0; 
...
bool run_sequence(const TimeStep * sequence, const int sequence_size)
{
    ...
    uInt32 *time_stamp = malloc(sequence_size * sizeof(uInt32));
    float64 *led_voltage = malloc(sequence_size * sizeof(float64));
    ...
    uInt8 *do_samples = malloc(2 * sequence_size * sizeof(uInt8));
    ...
    // DAQmx task init
    print_log(INFO, "DAQmx - Task init");
    DAQmxErrChk(DAQmxCreateTask("stimseq Digital Outputs", &task_handle_do));
    DAQmxErrChk(DAQmxCreateTask("stimseq Trigger", &task_handle_trig));
    ...
    // Enable tasks
    print_log(INFO, "DAQmx - Enable tasks");
	DAQmxErrChk(DAQmxStartTask(task_handle_do));
	DAQmxErrChk(DAQmxStartTask(task_handle_trig));
	DAQmxErrChk(DAQmxStartTask(task_handle_ao));

    // Wait for trigger signal from TTL input
    ...
    print_log(INFO, "Trigger Signal received");
    ...
    print_log(INFO, "Free Memory and return");
    free(time_stamp);
    free(led_voltage);
    free(do_samples);
...
// Load Sequence on the DAQ to be executed by it directly
bool load_sequence(const TimeStep * sequence, const int sequence_size)
{
    ...
    print_log(INFO, "DAQmx - Setup Sample generation timing");
    ...
    print_log(INFO, "DAQmx - Setup trigger signal");
    ...
    print_log(INFO, "DAQmx - Setup sample Generations");
    ...
    // Enable task
    print_log(INFO, "DAQmx - Enable task");
	DAQmxErrChk(DAQmxStartTask(task_handle_do));
	DAQmxErrChk(DAQmxStartTask(task_handle_ao));

    // Wait for the tasks to end
    print_log(INFO, "DAQmx - Wait for the tasks to end");
...
// cross-platform sleep function in miliseconds
static void sleep_ms(int milliseconds)
{ 
#ifdef WIN32
    Sleep(milliseconds);
